...
const UNITS = [
  ...
  { id: 'siege',  name: 'SIEGE WALKERS',role: 'siege', era: 2, slots: 3, cost: 1010, n: 2, hp: 240, dmg: 57, minR: 3, maxR: 9, br: [5,7,9],
    speed: 0.32, ap: 1, save: 6, splash: 2, acc: 1, emplaced: true,
    /* Trimmed rather than repriced. The marginal tuner is right that this gun is worth what it
       measures, but a fair price is not the same as a reachable one: at 1295 for three slots it
       was first fielded in round ten of a war that averages nine, so it was correctly valued and
       never bought. Value per credit is unchanged; the absolute ticket is 22% smaller. */
...
function enemyArmy(state, rng, opts) {
  ...
  /* Ranking and teching are separated from buying so a caller can run the purchase alone.
     Without this split any "let the regime buy twice" experiment silently also grants it a second
     tech roll and six more rank-ups, and measures two buying passes rather than better
     information — which is exactly how one such experiment was wasted. */
  if (opts && opts.buyOnly) return state.efleet;
...
function resolveRound(state, result) {
  ...
  if (state.supply <= 0) { state.over = true; state.won = false; }
    ...
    const em = state.mods || {};
    state.eCredits += Math.round(((em.enemyBase || 465) + (state.round - 1) * (em.enemySlope || 158)) * (em.enemyIncome || 1));   // round has already advanced
