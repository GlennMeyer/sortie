...
const MODULES = [
  { id: 'beam_rifle', name: 'BEAM RIFLE',     kind: 'w', mass: 12, power: 30, heat: 12, dmg: 95,  dtype: 'beam',    minR: 2, maxR: 7, br: [3, 5, 7], ap: 2, acc: -1, mount: 'R ARM', blast: 0, ammo: null, use: 0, cycle: 1, los: true,
    note: 'Reaches almost the whole board, needs a clear line, and cooks you if you hold the trigger.' },
  { id: 'beam_saber', name: 'BEAM SABER',     kind: 'w', mass: 6,  power: 20, heat: 16, dmg: 165, dtype: 'beam',    minR: 1, maxR: 1, br: [1, 1, 1], ap: 2, acc: 0, mount: 'R ARM', blast: 0, ammo: null, use: 0, cycle: 1, los: true,
    note: 'Adjacent hex only. Cheapest kill in the game — you have to survive the walk in.' },
  { id: 'mg',         name: 'MACHINE CANNON', kind: 'w', mass: 9,  power: 4,  heat: 4,  dmg: 32,  dtype: 'kinetic', minR: 1, maxR: 4, br: [2, 3, 4], ap: 0, acc: 0, mount: 'L ARM', blast: 0, ammo: 320, use: 8, cycle: 1, los: true,
    note: 'Almost no heat, short reach. Runs dry exactly when you need it.' },
  { id: 'missiles',   name: 'MISSILE POD',    kind: 'w', mass: 16, power: 6,  heat: 14, dmg: 68,  dtype: 'kinetic', minR: 3, maxR: 8, br: [5, 7, 8], ap: 1, acc: 1, mount: 'TORSO', blast: 1, ammo: 30,  use: 3, cycle: 2, los: false,
    note: 'Indirect fire — arcs over debris, no line of sight needed. Splashes one hex out, so it only pays against a cluster.' },
  { id: 'bazooka',    name: 'BAZOOKA',        kind: 'w', mass: 14, power: 3,  heat: 19, dmg: 285, dtype: 'kinetic', minR: 2, maxR: 6, br: [3, 5, 6], ap: 2, acc: 1, mount: 'TORSO', blast: 0, ammo: 10,  use: 1, cycle: 3, los: true,
    note: 'Ten rounds. Bring them to the rival, not the chaff.' },
  { id: 'scatter',    name: 'SCATTER GUN',    kind: 'w', mass: 11, power: 5,  heat: 12, dmg: 68,  dtype: 'kinetic', minR: 1, maxR: 2, br: [1, 2, 2], ap: 0, acc: -1, mount: 'L ARM', blast: 1, ammo: 56,  use: 2, cycle: 1, los: true,
  ...
  { id: 'bshield',    name: 'BEAM SHIELD',    kind: 's', mass: 10, power: 18, saveBonus: 1, passiveHeat: 3.2,
    note: 'Improves your armour save by one — while adding to the heat problem.' },
...
const ENEMIES = {
  grunt: { name: 'GRUNT',      gunnery: 5, save: 0, ap: 0, armor: 80,  dmg: 3.6,  minR: 1, maxR: 4, speed: 1.0, evade: 0.05 },
  arty:  { name: 'ARTILLERY',  gunnery: 4, save: 0, ap: 1, armor: 65,  dmg: 13.5,   minR: 3, maxR: 9, speed: 0.5, evade: 0.02,
           note: 'Cannot fire inside two hexes. Getting under it is the whole answer.' },
  skirm: { name: 'SKIRMISHER', gunnery: 5, save: 0, ap: 0, armor: 64,  dmg: 4.0,  minR: 1, maxR: 3, speed: 1.7, evade: 0.22, beamResist: 0.40 },
  elite: { name: 'LINE MODEL', gunnery: 4, save: 5, ap: 1, armor: 150, dmg: 6.1,   minR: 1, maxR: 5, speed: 0.9, evade: 0.12, beamResist: 0.25 },
  ace:   { name: 'ACE',        gunnery: 2, save: 4, ap: 2, armor: 760, dmg: 26,   minR: 1, maxR: 7, speed: 1.8, evade: 0.32 },
};
...
/* ---------- the ruleset ----------
   Everything resolves on visible dice. To hit: 2d6 against a target number built from a stack of
   modifiers the player can read before the roll. Damage lands on a location, and armour is a
   save you make rather than a number that quietly subtracts. */

const d6 = rng => 1 + Math.floor(rng() * 6);
const d66 = rng => d6(rng) + d6(rng);

/* 2d6 odds, for the AI to reason about a shot without rolling it */
const ODDS_2D6 = { 2: 1, 3: .972, 4: .917, 5: .833, 6: .722, 7: .583, 8: .417, 9: .278, 10: .167, 11: .083, 12: .028 };
const oddsOf = n => ODDS_2D6[clamp(Math.round(n), 2, 12)];

/* Gunnery: lower is better. Nerve makes a pilot steady; being green, hurt or exhausted does not. */
function gunnery(u) {
  let g = 4;
  if (u.nerve >= 78) g -= 1;
  if (u.pilot && u.pilot.temperament === 'green') g += 1;
  const strain = (u.trauma || 0) + (u.fatigue || 0);
  if (strain > 140) g += 2; else if (strain > 90) g += 1;
  return clamp(g, 1, 7);
}

/* Heat is no longer a switch. It degrades gunnery and movement long before it scrams you. */
const HEAT_SCALE = [
  { at: 0.00, name: 'NOMINAL',  toHit: 0, move: 0 },
  { at: 0.25, name: 'WARM',     toHit: 1, move: 0 },
  { at: 0.50, name: 'HOT',      toHit: 2, move: 1 },
  { at: 0.70, name: 'CRITICAL', toHit: 3, move: 2 },
  { at: 0.85, name: 'REDLINE',  toHit: 4, move: 3, shutdownOn: 8 },
];
function heatBand(frac) {
  let b = HEAT_SCALE[0];
  for (const x of HEAT_SCALE) if (frac >= x.at) b = x;
  return b;
}

const LOCATIONS = ['HEAD', 'TORSO', 'L ARM', 'R ARM', 'L LEG', 'R LEG'];
const LOC_SHARE = { HEAD: 0.08, TORSO: 0.34, 'L ARM': 0.12, 'R ARM': 0.12, 'L LEG': 0.17, 'R LEG': 0.17 };
function hitLocation(roll) {
  if (roll === 2 || roll === 12) return 'HEAD';
  if (roll <= 4) return 'R ARM';
  if (roll === 5) return 'R LEG';
  if (roll <= 8) return 'TORSO';
  if (roll === 9) return 'L LEG';
  return 'L ARM';
}

/* Armour save by chassis weight, worsened by the weapon's AP. 7+ means no save at all. */
const SAVE_BY_CLASS = { LIGHT: 5, MEDIUM: 4, HEAVY: 3 };

/* The modifier stack. Returned so it can be shown verbatim in the combat log. */
function toHitTarget(att, w, tgtHex, tgtMoved, map, isMech, defMod) {
  const mods = [];
  let n = att.gun == null ? gunnery(att) : att.gun;
  mods.push({ why: 'gunnery', v: n });

  const dist = map.d(att.hex, tgtHex);
  const br = w.br || [w.maxR, w.maxR, w.maxR];
  ...
  const am = att.movedThisTick >= 3 ? 2 : att.movedThisTick > 0 ? 1 : 0;
  if (am) { mods.push({ why: 'you moved', v: am }); n += am; }
  const tm = tgtMoved >= 3 ? 2 : tgtMoved > 0 ? 1 : 0;
  if (tm) { mods.push({ why: 'target moved', v: tm }); n += tm; }

  const cov = H.coverAt(map, att.hex, tgtHex);
  const cm = cov >= 0.2 ? 2 : cov > 0 ? 1 : 0;
  if (cm) { mods.push({ why: cm === 2 ? 'heavy cover' : 'partial cover', v: cm }); n += cm; }

  const hiA = map.terrain(att.hex).high, hiT = map.terrain(tgtHex).high;
  if (hiA && !hiT) { mods.push({ why: 'high ground', v: -1 }); n -= 1; }
  if (hiT && !hiA) { mods.push({ why: 'firing uphill', v: 1 }); n += 1; }

  if (isMech) {
    const hb = heatBand(att.heat / att.heatCap);
    if (hb.toHit) { mods.push({ why: 'heat ' + hb.name.toLowerCase(), v: hb.toHit }); n += hb.toHit; }
  }
  if (defMod) { mods.push({ why: 'evasive target', v: defMod }); n += defMod; }
  if (w.acc) { mods.push({ why: w.name.toLowerCase(), v: w.acc }); n += w.acc; }

  return { need: clamp(n, 2, 12), mods, bracket, dist };
}
function modsText(t) {
  return t.mods.map(m => m.why + (m.v > 0 ? ' +' + m.v : m.v < 0 ? ' ' + m.v : ' ' + m.v)).join(', ');
}
...
function makeUnit(entry, idx) {
  ...
  return {
    ...
    heatCap: st.heatCap, heat: 0, prop: st.propMax, hex: -1, mp: 0, movedThisTick: 0,
    saveBase: Math.max(2, SAVE_BY_CLASS[st.chassis.cls]
      - (st.mods.some(m => m.id === 'plating') ? 1 : 0) - (st.saveBonus || 0)),
    loc: (() => {
      const total = st.armorMax * (entry.integrity == null ? 1 : entry.integrity), o = {};
      for (const L of LOCATIONS) o[L] = total * LOC_SHARE[L];
      return o;
    })(),
    locMax: (() => { const o = {}; for (const L of LOCATIONS) o[L] = st.armorMax * LOC_SHARE[L]; return o; })(),
    lost: {},
...
/* Damage lands somewhere specific. An arm takes the weapon with it; legs take your speed;
   the head and the centre torso end the sortie. */
function hitLoc(u, loc, dmg, ev, units) {
  u.loc[loc] = Math.max(0, u.loc[loc] - dmg);
  u.armor = LOCATIONS.reduce((a, L) => a + u.loc[L], 0);
  if (u.loc[loc] > 0 || u.lost[loc]) return;
  u.lost[loc] = true;
  if (loc === 'L ARM' || loc === 'R ARM') {
    const gone = u.weapons.filter(w => !w.dead && (w.m.mount || 'TORSO') === loc);
    for (const w of gone) w.dead = true;
    ev.push({ k: 'crit', u: u.idx, s: `${u.pilot.name} — ${loc} BLOWN OFF` +
      (gone.length ? ', lost ' + gone.map(w => w.m.name).join(' and ') : '') });
  } else if (loc === 'L LEG' || loc === 'R LEG') {
    ev.push({ k: 'crit', u: u.idx, s: `${u.pilot.name} — ${loc} CRIPPLED, movement halved` });
  } else {
    ev.push({ k: 'crit', u: u.idx, s: `${u.pilot.name} — ${loc} DESTROYED` });
  }
}
function legPenalty(u) {
  const n = (u.lost['L LEG'] ? 1 : 0) + (u.lost['R LEG'] ? 1 : 0);
  return n === 0 ? 1 : n === 1 ? 0.5 : 0.15;
}
    ...
    while (waveTicks < 160) {
      ...
      for (const u of units) {
        ...
        if (u.shutdown <= 0) {
          ...
          /* move */
          if (bestHex !== u.hex) {
            const hbMove = heatBand(u.heat / u.s.heatCap);
            u.mp += Math.max(0.30, hexSpeed(u.s) * legPenalty(u) - hbMove.move * 0.35) * (u.prop > 0 ? 1 : 0.5);
            let guard = 0;
            while (guard++ < 6) {
              ...
              if (u.mp < cost) break;
              occ.delete(u.hex); u.mp -= cost; u.hex = step; occ.add(u.hex); u.moved++; u.movedThisTick++;
        ...
        if (u.shutdown > 0) {
          ...
          const nearest = enemies.filter(e => e.hp > 0)
            .sort((a, b) => map.d(u.hex, a.hex) - map.d(u.hex, b.hex))[0];
          const value = w => {
            if (!nearest) return 0;
            const th = toHitTarget(u, w.m, nearest.hex, nearest.movedThisTick || 0, map, true,
              Math.round((nearest.e.evade || 0) * 6));
            return (w.m.dmg * (1 + w.m.blast) * oddsOf(th.need)) / Math.max(w.m.heat, 0.5);
          };
          const ready = u.weapons
            .filter(w => !w.dead && w.cd <= 0 && (w.ammo === null || w.ammo >= w.m.use))
          ...
          for (const w of ready) {
            ...
            for (const tgt of aim.targets) {
              const defMod = Math.round((tgt.e.evade || 0) * 6);
              const th = toHitTarget(u, w.m, tgt.hex, tgt.movedThisTick || 0, map, true, defMod);
              const roll = d66(rng);
              ...
              if (roll < th.need) {
                ev.push({ k: 'miss', u: u.idx, shot: Object.assign({ missed: true }, shot),
                  s: `${u.pilot.name}: ${w.m.name} needs ${th.need}+ — rolled ${roll}, miss` , detail: modsText(th) });
                continue;
              }
              if (tgt.e.save) {                                  // armoured targets get a save
                const need = tgt.e.save + w.m.ap, sv = d6(rng);
                if (need <= 6 && sv >= need) {
                  ev.push({ k: 'miss', u: u.idx, shot: Object.assign({ missed: true }, shot),
                    s: `${w.m.name} hit ${tgt.e.name} — armour saved on ${sv} (needed ${need}+)` });
                  continue;
                }
              }
              let d = w.m.dmg;
        ...
        for (const e of enemies) {
          ...
          if (needsMove) {
            ...
            while (guard++ < 4 && e.mp >= 1) {
              ...
              const cost = map.terrain(step).cost;
              if (e.mp < cost) break;
              occ.delete(e.hex); e.mp -= cost; e.hex = step; occ.add(e.hex); e.movedThisTick = (e.movedThisTick || 0) + 1;
            }
          }
          ...
          let evd = Math.round(tgt.s.evade * 6) + (tgt.anchor > 0 ? 2 : 0);
          if (tgt.shutdown > 0 || tgt.prop <= 0) evd = 0;       // shut down or dry: a sitting target
          const ew = { name: e.e.name, br: [Math.ceil(e.e.maxR / 3), Math.ceil(e.e.maxR * 2 / 3), e.e.maxR],
            maxR: e.e.maxR, acc: 0, ap: e.e.ap || 0 };
          const att = { hex: e.hex, movedThisTick: e.movedThisTick || 0, gun: e.e.gunnery };
          const th = toHitTarget(att, ew, tgt.hex, tgt.movedThisTick || 0, map, false, evd);
          const roll = d66(rng);
          if (roll < th.need) {
            ev.push({ k: 'efire', shot: { from: e.hex, to: tgt.hex, dtype: 'enemy', blast: 0, side: 'e', missed: true } });
            continue;
          }
          const saveNeed = tgt.saveBase + (e.e.ap || 0);
          if (saveNeed <= 6 && d6(rng) >= saveNeed) {
            ev.push({ k: 'efire', shot: { from: e.hex, to: tgt.hex, dtype: 'enemy', blast: 0, side: 'e', missed: true } });
            continue;
          }
          const loc = hitLocation(d66(rng));
          const blk = tgt.anchor > 0 ? 0.25 : 0;
          const taken = e.e.dmg * (1 - blk);
          hitLoc(tgt, loc, taken, ev, units);
          ev.push({ k: 'efire', shot: { from: e.hex, to: tgt.hex, dtype: 'enemy', blast: 0, side: 'e', dmg: Math.round(taken), loc } });
      ...
      for (const u of units) {
        ...
        const hb = heatBand(u.heat / u.s.heatCap);
        u.heatBand = hb.name;
        if (u.heat >= u.s.heatCap && u.shutdown <= 0) {
          u.shutdown = 3; u.overheats++; u.heat = u.s.heatCap;
          ev.push({ k: 'crit', u: u.idx, s: `${u.pilot.name} — HEAT ${Math.round(u.heat)}/${u.s.heatCap}, forced shutdown` });
        } else if (hb.shutdownOn && u.shutdown <= 0) {
          const r2 = d66(rng);
          if (r2 < hb.shutdownOn) {
            u.shutdown = 3; u.overheats++;
            ev.push({ k: 'crit', u: u.idx, s: `${u.pilot.name} — REDLINE, shutdown check ${hb.shutdownOn}+ rolled ${r2}, reactor scrams` });
          } else ev.push({ k: 'warn', u: u.idx, s: `${u.pilot.name} — redline, holds it together on ${r2}` });
        }
      ...
      for (const u of units) {
        if (u.dead) continue;
        if (u.loc['TORSO'] > 0 && u.loc['HEAD'] > 0 && u.armor > 0) continue;
        u.cause = u.loc['HEAD'] <= 0 ? 'COCKPIT HIT' : 'TORSO BREACH';
        u.dead = true; u.deadAt = t; u.armor = 0;
        if (u.prop <= 0) u.cause = 'OUT OF PROPELLANT';
        else if (u.overheats > 1) u.cause = 'THERMAL COLLAPSE';
      ...
      if (!opts.noFrames) frames.push({
        ...
        units: units.map(u => ({
          ...
          loc: Object.assign({}, u.loc), locMax: u.locMax, lost: Object.assign({}, u.lost),
          heatBand: heatBand(u.heat / u.s.heatCap).name, save: u.saveBase, gunnery: gunnery(u),
...
const ARCHETYPES = [
  { key: 'knife',  name: 'KNIFE FIGHTER', doctrine: 'measured',   chassis: 'halberd', mods: ['beam_saber', 'scatter', 'radiator', 'thruster'],
    blurb: 'Everything fires at short range, so nothing takes a range penalty.' },
  { key: 'breaker',name: 'LINE BREAKER',  doctrine: 'aggressive', chassis: 'bulwark', mods: ['beam_rifle', 'beam_saber', 'radiator', 'plating', 'hopper'],
    blurb: 'Answers at any bracket and saves on 2+. Slow, and does not care.' },
  { key: 'duel',   name: 'HEAVY DUELIST', doctrine: 'measured',   chassis: 'bulwark', mods: ['beam_saber', 'scatter', 'radiator', 'thruster', 'plating'],
    blurb: 'Walks in behind a 2+ save and takes the arms off things.' },
  { key: 'interc', name: 'INTERCEPTOR',   doctrine: 'measured',   chassis: 'wasp',    mods: ['beam_saber', 'scatter', 'plating'],
    blurb: 'No radiator. Redlines about once a sortie and rolls to stay up.' },
  { key: 'line',   name: 'LINE MECH',     doctrine: 'aggressive', chassis: 'halberd', mods: ['beam_rifle', 'scatter', 'radiator', 'thruster', 'hopper'],
    blurb: 'The doctrinal answer: a bracket for every range.' },
  { key: 'sniper', name: 'SNIPER',        doctrine: 'measured',   chassis: 'wasp',    mods: ['beam_rifle', 'missiles', 'hopper'],
    blurb: 'Stays at long range and lets indirect fire do the rest.' },
];
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CHASSIS, MODULES, MOD, ENEMIES, WAVES, DOCTRINES, ARCHETYPES, TEMPERAMENTS, SIGNATURES, AUTOPILOT,
    ...
    LOCATIONS, LOC_SHARE, HEAT_SCALE, SAVE_BY_CLASS, gunnery, heatBand, hitLocation, toHitTarget, modsText, oddsOf, d6, d66, mulberry32, clamp, H };
}
